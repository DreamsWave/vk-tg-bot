...
describe('APIs tests', () => {
        ...
        describe('send', () => {
            ...
            it('should send message with 2 photos', async () => {
                ...
                const media = await prepareMedia(
                    [getAttachment('photo', 'small'), getAttachment('photo', 'small')],
            ...
            it('should send message with gif and pdf', async () => {
                ...
                const media = await prepareMedia(
                    [getAttachment('doc', 'gif'), getAttachment('doc', 'pdf')],
                    { randomFilenames: true, saveTo: tmpDir }
                );
                try {
                    await tg.send(text, media, { disable_notification: true });
    ...
    describe('Yandex Cloud API tests', () => {
        describe('Yandex Message Queue', () => {
            beforeAll(() => {
                jest
                    .spyOn(ymq, 'sendMessage')
                    .mockImplementation(() =>
                        Promise.resolve({} as SQS.SendMessageResult)
                    );
            });
